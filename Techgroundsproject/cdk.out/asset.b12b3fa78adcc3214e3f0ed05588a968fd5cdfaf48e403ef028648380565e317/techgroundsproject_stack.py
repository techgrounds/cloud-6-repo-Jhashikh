...
class TechgroundsprojectStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
    ...
    )   # backup_vault: backup.BakcupVault
        backup.BackupVault(self, 'Vault',
            block_recovery_point_deletion=False,
            removal_policy=RemovalPolicy.DESTROY,
            )
        websvr_vault= backup.BackupVault.from_backup_vault_name(self, 'Vault', 'websvrVault')
        plan.add_rule(backup.BackupPlanRule(websvr_vault,
      ...
      # plan: backup.BackupPlan
        ...
        mgmtsvr_vault= backup.BackupVault.from_backup_vault_name(self, 'Vault', 'mgmtsvrVault')
        plan1.add_rule(backup.BackupPlanRule(mgmtsvr_vault,
            completion_window=Duration.hours(2),
            start_window=Duration.hours(1),
            schedule_expression=event.Schedule.cron(
                week_day='1',
